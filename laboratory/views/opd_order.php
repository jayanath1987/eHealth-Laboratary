...
<?php echo Modules::run('menu'); //runs the available menu option to that usergroup ?>
	<div class="row" style="margin-top: 55px; padding-bottom: 10px; padding-top: 15px;">
	              <table border="0" width="100%" >
                    <tr >
                        <td valign="top" class="rightmaintable" style="padding-left: 35px;">
		...
		<div class="panel panel-default  "  style="padding:2px;margin-bottom:1px;" >
					...
					echo '<table class="table table-condensed table-hover" style="margin-bottom:0px;">';
						...
						echo '<tr>';
							...
							echo '<td>';							
								if (isset($opd_lab_info["CreateDate"])){
									echo 'Order Date : <b>'.$opd_lab_info["OrderDate"].'</b>';
						...
						echo '<tr>';
								...
								if (isset($oreder_info)){
									echo $oreder_info["OrderDate"];
								}
                                                                
								if (($mode!="view")&&($mode!="process")){
									echo '<input  type="text" id="datepicker"  value="'.date("Y-m-d").'"  onchange=$("#OrderDate").val(this.value) o />';
								}
							echo '</td>';
							echo '<td>';
								echo '<b>Priority:</b>';
								...
								if (($mode!="view")&&($mode!="process")){
									echo '<select onchange=$("#Priority").val(this.value)  class="form-control input-sm"><option value="Normal">Normal</option><option value="Urgent">Urgent</option></select>';
								}
							echo '</td>';						
							echo '<td >';							
								echo '<b>Test group:</b>';
						...
						echo '</tr>';
					echo '</table>';	
					echo '<br><center><a href="'.site_url("opd/view/".$opd_visits_info["OPDID"]).'">Back to visit</a></center>';	
					echo '<div>';
					echo '<table class="table table-condensed table-hover" style="margin-bottom:0px;">';
					if (isset($episode_lab_order)){
						...
						foreach($episode_lab_order as $key=>$value) {
							echo '<tr>';
								...
								echo '</td>';
								echo '<td>';
								echo ' <a  href="'.site_url("laboratory/order/".$value["LAB_ORDER_ID"]."/?CONTINUE=laboratory/opd_order/".$opd_visits_info["OPDID"]).'">[Open]&nbsp;</a>';
								echo ' <a target="_blank" href="'.site_url("laboratory/order/".$value["LAB_ORDER_ID"]."/?CONTINUE=laboratory/opd_order/".$opd_visits_info["OPDID"]).'">[Print]&nbsp;</a>';
								echo ' <a  href="'.site_url("laboratory/process_result/".$value["LAB_ORDER_ID"]."/?CONTINUE=laboratory/opd_order/".$opd_visits_info["OPDID"]).'">[Enter result]</a>';
								echo '</td>';
			...
			?>" >
				<?php
					if ($mode=="process"){
						echo '<input type="hidden" name="CONTINUE" id="CONTINUE" value="'.(isset($_GET["CONTINUE"])?$_GET["CONTINUE"]:"").'" >';
					}
					else {
					...
					<div class="panel panel-default  "  style="padding:2px;margin-bottom:1px;" >
						...
						<div id="lab_cont">
										...
										foreach($orederd_test_list as $key=>$value) {
											echo '<tr>';
												...
												if ($value["Status"] == "Pending"){
													if ($mode=="process"){
														echo '<td>';
															echo '<input id="'.$value["LAB_ORDER_ITEM_ID"].'"  name="'.$value["LAB_ORDER_ITEM_ID"].'"   cols=40 rows=4>';
														echo '</td>';
                                                                                                                 if ($value["RefValue"]){
                                                                                                                echo "<td valign=top class='lab_td' width=200 ><span style='cursor:pointer;color:green;' onclick=$('#ref".$i."').toggle()>&raquo;</span><div id='ref".$i."' style='display:none;font-size:10px;'>". $value["RefValue"]."</div>&nbsp;</td>";
                                                                                                                 }
														/*echo '<td>';
														echo $value["RefValue"];
														echo '</td>';*/
							...
							?>
                                          ...
                                          <td valign="top" class="leftmaintable" style="padding-right: 20px;">
